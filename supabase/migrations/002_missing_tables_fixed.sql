...
CREATE TABLE IF NOT EXISTS fechamentos_eventos (
    ...
    status VARCHAR DEFAULT 'rascunho' CHECK (status IN ('rascunho', 'aprovado', 'revisao')),
...
-- Inserir perfis padrão (usando INSERT ... WHERE NOT EXISTS para evitar duplicatas)
DO $$
BEGIN
...
-- Inserir permissões básicas (usando INSERT ... WHERE NOT EXISTS)
DO $$
DECLARE
    perm RECORD;
    permissoes_data TEXT[][] := ARRAY[
    ...
    i INTEGER;
BEGIN
    FOR i IN 1..array_length(permissoes_data, 1) LOOP
        IF NOT EXISTS (SELECT 1 FROM permissoes WHERE nome = permissoes_data[i][1]) THEN
...
BEGIN
    ...
    IF admin_id IS NOT NULL THEN
        ...
        WHERE NOT EXISTS (
            ...
            WHERE pp.perfil_id = admin_id AND pp.permissao_id = p.id
        );
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS update_fechamentos_eventos_updated_at ON fechamentos_eventos;
...
DROP POLICY IF EXISTS "Usuários autenticados podem ver permissões" ON permissoes;
DROP POLICY IF EXISTS "Usuários autenticados podem ver perfil_permissões" ON perfil_permissoes;
DROP POLICY IF EXISTS "Usuários podem ver seus próprios dados" ON users;
DROP POLICY IF EXISTS "Usuários autenticados podem ver entidades" ON entidades;
DROP POLICY IF EXISTS "Usuários autenticados podem gerenciar tickets" ON tickets;
DROP POLICY IF EXISTS "Usuários autenticados podem ver fechamentos" ON fechamentos_eventos;
